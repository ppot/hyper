...
class TermsComponent extends React.Component {
  ...
  renderNode(termNode, sizes, position, displayDivider, direction, activeEvent, activeNode) {
  ...
  renderNodesTerms(termsNode, termNode, terms, term, activeEvent) {
      ...
      >
        {
          _nodes.map((t, i) => {
            ...
            if (node != null) {
              return this.renderNodesTerms(node, term[t], terms, term, activeEvent);
            } else {
              const displayDivider = i < _nodes.length - 1;
              console.log(displayDivider);
              return this.renderNode(term[t], sizes, i, displayDivider, termsNode.direction, activeEvent, termsNode.uid);
            }
          })
        }
        <style jsx>{`
    ...
    return (
      ...
      {rootNodes.map((t, i) => {
        if (_nodes[t] != null && _nodes[t].parent != null) {
          return this.renderNodesTerms(_nodes[t], term[t], terms, term, activeEvent);
          
        } else {
          const displayDivider = i < rootNodes.length - 1;
          console.log(displayDivider, i , rootNodes.length );
          return this.renderNode(term[t], sizes, i, displayDivider, root.direction, activeEvent, root.uid);
        }
      })}
      <style jsx>{`
        .panes {
