...
class TermsComponent extends React.Component {
  ...
  renderNode(termNode, sizes, position, displayDivider, direction, activeEvent, activeNode, render) {
  ...
  renderNodesTerms(termsNode, termNode, terms, term, activeEvent, render) {
      ...
      >
        {
          _nodes.map((t, i) => {
            ...
            if (node != null) {
              return this.renderNodesTerms(node, term[t], terms, term, activeEvent, render);
            } else {
              const displayDivider = i < _nodes.length - 1;
              console.log(displayDivider);
              return this.renderNode(term[t], sizes, i, displayDivider, termsNode.direction, activeEvent, termsNode.uid, render);
            }
          })
        }
        <style jsx>{`
  ...
  renderTerms(terms, term, activeEvent, render) {
    ...
    return (
      ...
      {rootNodes.map((t, i) => {
        if (_nodes[t] != null && _nodes[t].parent != null) {
          return this.renderNodesTerms(_nodes[t], term[t], terms, term, activeEvent, render);
          
        } else {
          const displayDivider = i < rootNodes.length - 1;
          console.log(displayDivider, i , rootNodes.length );
          return this.renderNode(term[t], sizes, i, displayDivider, root.direction, activeEvent, root.uid, render);
        }
      })}
      <style jsx>{`
        .panes {
  ...
  render() {
    const {activeTab, terms, term, onActive, render} = this.props;
    const _terms = terms[activeTab];
    return (
      <div className={`terms`}>
        {_terms != null && this.renderTerms(_terms, term, onActive, render)}
        <StyleSheet_ />
        <style jsx>{`
          .terms {
            position: absolute;
