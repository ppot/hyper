...
class TermsComponent extends React.Component {
  ...
  render() {
    const {activeTab, terms, layouts, layout, termsNodes, term, sessions, onData, onActive, onResize, render} = this.props;
    return (
      <div className={`terms`}>
        {layout !=null && this.renderLayout(layout, render)}
        <StyleSheet_ />
        <style jsx>{`
          .terms {
            position: absolute;
