...
class TermsComponent extends React.Component {
  ...
  renderNode(termNode, sizes, position, displayDivider, direction, sessions, onData, activeEvent, activeNode, render) {
  ...
  renderNodesTerms(termsNode, termNode, terms, term, sessions, onData, activeEvent, render) {
      ...
      >
        {
          _nodes.map((t, i) => {
            ...
            if (node != null) {
              return this.renderNodesTerms(node, term[t], terms, term, sessions, onData, activeEvent, render);
            } else {
              const displayDivider = i < _nodes.length - 1;
              console.log(displayDivider);
              return this.renderNode(term[t], sizes, i, displayDivider, termsNode.direction, sessions, onData, activeEvent, termsNode.uid, render);
            }
          })
        }
        <style jsx>{`
  ...
  renderTerms(terms, term, sessions, onData, activeEvent, render) {
    const _nodes = terms.nodes;
    const root = _nodes[terms.root];
    console.log(root);
    if (root != null && root.nodes.length === 1) {
      const _term = {
        term : term[_nodes[terms.root].nodes[0]],
        session : sessions[_nodes[terms.root].nodes[0]],
        onData,
        activeEvent,
        activeNode: term[_nodes[terms.root].nodes[0]],
        render
      };
      return (
        <div
    ...
    return (
      ...
      {rootNodes.map((t, i) => {
        if (_nodes[t] != null && _nodes[t].parent != null) {
          return this.renderNodesTerms(_nodes[t], term[t], terms, term, sessions, onData, activeEvent, render);
          
        } else {
          const displayDivider = i < rootNodes.length - 1;
          console.log(displayDivider, i , rootNodes.length );
          return this.renderNode(term[t], sizes, i, displayDivider, root.direction, sessions, onData, activeEvent, root.uid, render);
        }
      })}
      <style jsx>{`
        .panes {
  ...
  render() {
    const {activeTab, terms, term, sessions, onData, onActive, render} = this.props;
    const _terms = terms[activeTab];
    return (
      <div className={`terms`}>
        {_terms != null && this.renderTerms(_terms, term, sessions, onData, onActive, render)}
        <StyleSheet_ />
        <style jsx>{`
          .terms {
            position: absolute;
