...
class TermComponent extends React.PureComponent {
  ...
  componentDidMount() {
    const {props} = this;
    console.log(props);
    console.log('componentDidMount : ', props.uid);
    console.log(props);
    const termOptions = getTermOptions(props.render);
    if (terms[props.uid]) {
      this.xterm = terms[props.uid];
  ...
  onMouseUp(e) {
    this.props.onActive();
    // this.props.onActive(this.props.term.uid, this.props.activeNode);
