...
rpc.on('termgroup restore req', ({tab}) => {
  store_.dispatch(termGroupActions.restoreTermGroup(tab));
});

rpc.on('split restore', ({uid, split}) => {
  store_.dispatch(termGroupActions.restoreSplit(uid, split));
});
