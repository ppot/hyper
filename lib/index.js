...
import * as tabActions from './tab/tabActions';
import * as termsActions from './terms/termsActions';
import {addNotificationMessage} from './actions/notifications';
import {loadConfig, reloadConfig} from './actions/config';
// import HyperContainer from './containers/hyper';
import HyperContainer from './render/hyperContainer';
...
rpc.on('tab:created', ({uid}) => {
  store_.dispatch(tabActions.tabCreated(uid));
  store_.dispatch(termsActions.termsRequest(uid));
});

rpc.on('tab:active', ({uid}) => {
  store_.dispatch(tabActions.tabActive(uid));
  store_.dispatch(termsActions.termsActive(uid));
});

rpc.on('term:close', () => {
  store_.dispatch(termsActions.termClose());
});

rpc.on('term:request', ({uid, direcion}) => {
  console.log('term:request');
  store_.dispatch(termsActions.termRequest(uid, direcion));
});
...
rpc.on('split request horizontal', () => {
  ...
  // store_.dispatch(termsActions.requestHorizontalSplit());
});

rpc.on('split request vertical', () => {
  store_.dispatch(termGroupActions.requestVerticalSplit());
  // store_.dispatch(termsActions.requestVerticalSplit());
});
